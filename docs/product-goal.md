...
- 先识别页面角色，再按角色生成；封面、章节页、正文页、表格页和尾页允许版式不同，但必须共享同一字体气质、色彩系统和母版锚点。
- 同类内容页使用稳定的标题层级、页眉、页脚、Logo 和页码位置。页码只允许整套不显示、保留经确认的源页码，或按一种格式和一个位置统一规范化。
- 既有 PPT 的最后一页只有在内容语义确实为感谢、问答、联系方式或行动项时才是尾页；不得把报价、参数、产品或正文页改成尾页。
- 占位符、`Slide N` / `Page N` 模板字样、虚构品牌/Logo/网址/价格/日期/数字、正文大量缺失均属于交付阻断项。
- 人工通过必须绑定当前图片哈希与当前语义质检证据；图片、OCR 或规则变化后需要重新确认。
...
- 可编辑页预览与图片版页面通过自动视觉一致性检查，不能以大幅简化版式、丢失视觉结构的方式冒充重建成功。
- 没有整页截图冒充可编辑页面。
- 页面证据和最终证据完整。
- 人工视觉复核已记录。
...
- 上游 Skill 文件保持只读来源；产品层不得用旧模板、OCR 覆盖或自定义后处理改变 Skill 生成结果。
- 全套图片生成必须继承同一份已批准样张和结构化 StyleSpec，并在逐页提示词、图片引用和质量报告中保留证据。
- 旧 PPT 的封面、正文、表格、产品、总结和尾页角色必须按页面语义判断，不能按页序或历史模板标签机械改写。
...
## 可重复业务能力门禁（2026-08-19）

“单个任务交付 ready”不再等同于“产品业务能力完成”。工具级业务能力必须由 `npm run business:readiness` 独立判定，并同时满足：

- 至少 20 个非内部真实任务。
- 至少覆盖 4 类材料：PPT、PDF、图片组、brief；文档可作为额外类型。
- 完整产品交付成功率不低于 90%。
- 新版普通用户任务的可预测预算覆盖率为 100%。
- 成功任务平均失败恢复事件不高于 0.5 次。

新版普通用户入口会在大纲页数确定后显示任务费用上限、20% 风险缓冲和最大图片调用次数。模型单价未配置时允许继续整理材料和大纲，但禁止创建并授权付费任务；达到任务图片调用上限后必须停止，不能通过重复授权绕过。

## 2026-08-20 门禁口径与本地安全边界

- 工具级业务门禁只评估加入当前策略 `repeatable-business-v1` 的非内部、未归档真实任务；历史试验任务保留为工程证据，但不会永久污染新版 100% 预算覆盖率。
- 新任务创建时由后端自动持久化策略版本和加入时间，不允许前端伪造历史任务为当前 cohort。
- 恢复率只统计用户可感知的顶层重跑/恢复动作，不再把同一次失败派生的 worker task reset 和 batch failed 重复相加。
- 未变化任务的重型交付判定可以复用缓存；报告必须公开 cache hit/miss、总耗时和被排除的历史任务数量。
- 本地服务默认只绑定回环地址，并拒绝非回环 Host、未允许的浏览器 Origin 和跨站资源读取。当前版本不提供未经认证的局域网模式。
- `/api/health` 必须返回启动时与当前源码/构建指纹；两者不一致时前端明确提示重启，不能继续显示“已是最新运行版本”。
## 2026-06-30 当前产品边界补充
