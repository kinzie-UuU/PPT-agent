...
assert.ok(frontendSource.includes("全部通过并继续"));
...
assert.ok(indexSource.includes("assertWorkflowOcrTextHintsReady(currentJob)"));
...
assert.ok(workflowImageDeckReviewSource.includes("DOWNSTREAM_IMAGE_DECK_KEYS"));
...
assert.ok(workflowVisualsSource.includes("design_layer_"));
...
assert.ok(frontendSource.includes("onCreateWorkflow?.({ deliveryMode: selectedDeliveryMode })"));
...
assert.ok(frontendSource.includes("route-completion"));
assert.ok(frontendSource.includes("{job?.id ? ("));
...
assert.ok(frontendSource.includes("识别页面内容"));
assert.ok(frontendSource.includes("可确认大纲"));
assert.ok(frontendSource.includes("每页高清图片"));
